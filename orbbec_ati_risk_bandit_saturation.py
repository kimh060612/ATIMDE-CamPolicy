...
def configure_saturation_parser(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--sat-pixel-threshold", type=float, default=250.0)
    parser.add_argument("--sat-soft-clip-ratio", type=float, default=0.80)
    parser.add_argument("--sat-secondary-clip-ratio", type=float, default=0.85)
    parser.add_argument("--sat-hard-clip-ratio", type=float, default=0.90)
    parser.add_argument("--sat-hard-mean-luminance", type=float, default=245.0)
    parser.add_argument("--sat-recovery-clip-ratio", type=float, default=0.70)
    parser.add_argument("--sat-recovery-frames", type=int, default=3)
    parser.add_argument("--sat-quarantine-rounds", type=int, default=30)
    parser.add_argument("--sat-min-ev-drop-stops", type=int, default=1)


def validate_saturation_args(
    parser: argparse.ArgumentParser, args: argparse.Namespace
) -> None:
    try:
        SaturationGuardConfig.from_args(args)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    configure_saturation_parser(parser)
    parser.set_defaults(
    ...
    validate_saturation_args(parser, args)
    return args


def build_experiment(args: argparse.Namespace) -> RiskBanditSaturationExperiment:
    # Hardware/model imports stay lazy so tests need neither the SDK nor a GPU model.
    ...
    policy = SaturationGuardedRiskBanditPolicy(
        RiskBanditConfig.from_args(args), safety_policy, config.default_cell
    )
    ...
    guard = SaturationGuard(SaturationGuardConfig.from_args(args))
    return RiskBanditSaturationExperiment(
        config,
        capture_runner,
        predictor,
        policy,
...
def main(argv: Sequence[str] | None = None) -> int:
    experiment: RiskBanditSaturationExperiment | None = None
    exit_code = 0
    try:
