...
def configure_predictive_saturation_parser(
    parser: argparse.ArgumentParser,
) -> None:
    parser.add_argument("--sat-pixel-threshold", type=float, default=250.0)
    parser.add_argument("--sat-soft-clip-ratio", type=float, default=0.60)
    parser.add_argument("--sat-secondary-clip-ratio", type=float, default=0.85)
    parser.add_argument("--sat-hard-clip-ratio", type=float, default=0.89)
    parser.add_argument("--sat-hard-mean-luminance", type=float, default=243.0)
    parser.add_argument("--sat-recovery-clip-ratio", type=float, default=0.50)
    parser.add_argument("--sat-recovery-frames", type=int, default=3)
    parser.add_argument("--sat-quarantine-rounds", type=int, default=60)
    parser.add_argument("--sat-min-ev-drop-stops", type=int, default=1)
    parser.add_argument("--sat-max-upward-ev-stops", type=int, default=1)
    parser.add_argument(
        "--sat-projected-pixel-threshold", type=float, default=250.0
    )
    parser.add_argument(
        "--sat-projected-hard-clip-ratio", type=float, default=0.90
    )


def validate_predictive_saturation_args(
    parser: argparse.ArgumentParser, args: argparse.Namespace
) -> None:
    try:
        PredictiveSaturationGuardConfig.from_args(args)
...
def build_experiment(
    ...
    return RiskBanditPredictiveSaturationExperiment(
        config,
        capture_runner,
        predictor,
        policy,
...
def main(argv: Sequence[str] | None = None) -> int:
    experiment: RiskBanditPredictiveSaturationExperiment | None = None
    exit_code = 0
    try:
